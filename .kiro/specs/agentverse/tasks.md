...
- [ ] 28. 實作基礎成長系統
  ...
  - [ ] 28.3 AgentDex Stats 展示
    - AgentCard 顯示 W-L record + XP level
    - Badge 圖示（複用既有 badge PNG assets）
    - DoD: 視覺整合
  - [ ] 28.4 LLM Provider 設定 UI
    - Settings 頁面 / modal：API key 管理（Ref: MASTER_SPEC §16.8 — localStorage, never sent to server）
    - Provider 選擇（MVP: OpenAI only；interface 支援擴展）
    - DoD: 設定可存取，Arena 可使用
...
- [ ] B3. GenePack 交換流程（Phase 3）
  - **Why-defer**: 需要 Prompt Brawl 的 trial transcript 作為 extraction 輸入源，Phase 2 尚無實際對戰資料可驗證 extraction 品質
  - **Landing**: Phase 3（Tasks 25-28 完成且有真實對戰資料後）
  - GenePack MVP schema 已定義（見 PROJECT_MASTER_SPEC §17）
  - Extraction flow: Arena 結束後，本地端提取教練指令 → 結構化為 GenePack template → 上傳 metadata（不含明文對話）
  - 預設 private；public 需明確確認；交易功能 Phase 3+
  - **DoD**: 用戶可從完成的 Brawl match 提取 GenePack，metadata 上傳成功且不含明文對話
  - _需求：13, 14_
