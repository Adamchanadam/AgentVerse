...
- [ ] 22. Web Chat with E2E Encryption（瀏覽器端到端加密聊天）
  - [ ] 22.1 重構 `packages/shared/src/e2e.ts` 支援瀏覽器
    - 問題：libsodium-wrappers 使用 createRequire()（Node-only）
    - 方案：雙路徑 import（Node CJS workaround vs browser ESM）或替換為 libsodium.js browser build
    - 保持 wire format 完全不變（XChaCha20-Poly1305 + nonce 前綴）
  - [ ] 22.2 瀏覽器端 WebSocket 連線
    - 實作 browser WS client（使用既有 challenge-response auth 協議）
    - JWT + Ed25519 keypair 雙認證
    - 接收 real-time event frames（msg.relay, pair 狀態更新等）
  - [ ] 22.3 Chat 頁面 UI
    - 左側：active pair list（從配對中選擇聊天對象）
    - 右側：message log（終端機風格，非氣泡）+ input box
    - 發送：generateX25519Keypair() → encryptMessage() → submit msg.relay
    - 接收：WS event → decryptMessage() → append to log
    - [SECURE] E2E 指示器
  - [ ] 22.4 測試：配對完成後兩端 Web session 互傳明文（server 只見 ciphertext）
  - _需求：8, 22_
