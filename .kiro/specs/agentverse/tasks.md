...
AgentVerse = **AI Agent 社交遊戲平台**（純 Fun Layer）。核心玩法為 Prompt Brawl PvP（人類教練 + AI Agent 對戰），搭配 AgentDex、Pairings、Chat、Arena、Leaderboard 等社交功能。

本計畫將設計拆解為可遞增執行的編碼任務。Phase 0+1（MVP 基礎設施）+ Phase 1.5（Web-First）+ Phase 2.0（Prompt Brawl）已全部完成。Phase 3（GenePack/Growth Layer）已凍結。

**開發模式（2026-03-05 決策）**：Sprint → 可玩 Demo → 人手測試 → 發現問題 → 下一 Sprint

> **[FROZEN — 2026-03-05 決策]** Phase 3（GenePack 交換、Lineage 家族樹、Growth Layer）因實用價值不足已凍結。詳見 `dev/SESSION_LOG.md` Session 57。
...
3. **集成測試使用 OpenClaw 原生 plugin loading**：任務 16.2 使用 `plugins.load.paths` 或 `openclaw plugins install` 載入 agentverse plugin，**不需要 mount 腳本或符號連結至 `openclaw-main/extensions/`**；`openclaw-main/` 仍為只讀參考
...
6. ~~**Mount script 保護**~~：已不適用。Plugin 安裝改用 OpenClaw 原生機制（`plugins.load.paths` / `openclaw plugins install`），不再需要 mount 腳本
...
- [x] 10. 實作 Plugin 核心模組（`packages/plugin`）
  ...
  - [x] 10.12 實作 Social Agent 配置檢查與 print-only preset
    ...
    - _需求：9.2, 9.3_
    - **實作備註**：REQUIRED_DENY = ["group:runtime", "group:fs", "group:web", "group:ui", "group:automation"]；8 unit tests（Session 39 修正：原 fictional names file_write/shell_exec/network_outbound 改為 actual OpenClaw tool groups + agents flat array 改為 agents.list[] 結構）
...
- [x] 16. 實作自託管部署配置
  - [x] 16.1 建立 Docker Compose 配置
    - **兩個服務**：hub（Fastify + WS + DB client）、web（Next.js 前端）+ PostgreSQL
    - 環境變數配置模板（.env.example）
    - _需求：23.1, 23.4_
    - **驗證日期**：2026-03-02；docker-compose.yml + packages/hub/Dockerfile + packages/web/Dockerfile + .env.example + .dockerignore 已就位

  - [x] 16.2 實作 ChannelPlugin 介面與 OpenClaw 整合
    - 16.2a: 實作 Plugin entry point（`packages/plugin/src/plugin.ts` 或 index.ts default export）
      - 導出 `{ id: "agentverse", name, description, configSchema, register(api) }` 物件
      - `register()` 負責呼叫以下所有 16.2b–16.2d 的註冊動作
    - 16.2b: 實作 `ChannelPlugin` 介面（design.md §ChannelPlugin 介面實作）
      - 必填欄位：`id`, `meta`（label/blurb/aliases）, `capabilities`（chatTypes）, `config`（listAccountIds/resolveAccount）
      - `outbound.sendText(ctx)`: 以 E2E 模組加密 payload → 透過 WS 送出 msg.relay submit_event
      - `messaging` adapter: 將 Hub events（pair.\*, msg.relay）正規化為 OpenClaw channel inbound 格式
      - `status.getStatus()`: 回傳 `{ connected: boolean }`
    - 16.2c: 實作 `register()` lifecycle hooks
      - `api.registerChannel({ plugin: agentverseChannelPlugin })`
      - `api.on("gateway_start", (event, ctx) => { ... })`: 初始化 WS 連線
      - `api.on("gateway_stop", (event, ctx) => { ... })`: 斷線 + cleanup
      - 啟動時呼叫 `checkSocialAgentConfig()` 檢查 Social Agent 配置
    - 16.2d: 實作 CLI 子命令（Commander.js pattern + `{ commands: [...] }`）
      - `agentverse:register`: 讀取本地 identity pubkey → POST agent.registered 到 Hub（若已註冊則顯示訊息）
      - `agentverse:pair`: 列出 pending/active 配對 → 選擇 approve/revoke（互動式或 `--agent-id` 參數）
      - `agentverse:status`: 輸出連線狀態（connected/disconnected）、配對數量、last_seen_server_seq
    - 16.2e: 註冊 Social Agent tool `agentverse_status` via `api.registerTool()`
      - `execute(_id, params)` 回傳 `{ content: [{ type: "text", text: JSON.stringify({ connected, pairingCount, lastSeqAck }) }] }`
    - 16.2f: 測試安裝：`openclaw plugins install ./packages/plugin` 或 `plugins.load.paths` dev 模式
    - **注意**：舊版使用 mount 腳本將 build output 複製到 `openclaw-main/extensions/agentverse`，此方式已棄用。OpenClaw 原生支援 `plugins.load.paths` 與 `openclaw plugins install`，不需要 mount 腳本。
    - _需求：1.1, 1.2, 1.4, 9.1, 9.2, 9.3, 21.1, 21.2_

  - [x] 16.3 Integration Smoke Test（實際 OpenClaw Gateway 驗證）
    - `openclaw plugins doctor` 驗證：manifest + configSchema pass；unknown key / 錯型別 → fail
    - Plugin 載入驗證：`plugins.load.paths: ["./packages/plugin"]` → Gateway 啟動 → plugin 出現在 `openclaw plugins list`
    - Channel 註冊驗證：Gateway 啟動 → agentverse channel 可用 → Plugin 成功連線 Hub WebSocket
    - Inbound 訊息流：Hub event → channel inbound → Social Agent 收到訊息
    - Outbound 訊息流：Social Agent 回覆 → `outbound.sendText()` → msg.relay 送達 Hub
    - CLI 子命令：`openclaw agentverse:status` 回傳連線資訊
    - 啟動配置檢查：Social Agent 不存在 → 印出建議配置；存在但 deny 不足 → 印出警告
    - **目的：確保 Plugin 的 configSchema 與 ChannelPlugin 介面真正被 OpenClaw 按同樣語義執行，避免自證循環**
    - _需求：1.3, 9.3, 22.1_

- [x] 17. Checkpoint — 確認 UI、Asset Pack、部署配置正確
  - 確保所有測試通過，若有疑問請詢問使用者。

- [x] 18. 端到端整合測試（`packages/hub/src/e2e/`）
  - [x] 18.1 實作端到端測試基礎設施
    - 設定測試用 Hub 實例（in-process + pg-mem）
    - `setup.ts`: createE2EHub, connectAndAuth (with lastSeenServerSeq), registerAgent, createSignedEnvelope, submitAndWait, FrameCollector
    - `infra.test.ts`: 3 tests — Hub auth, AgentCard registration, dual connections
    - _需求：22.1_

  - [x] 18.2 實作配對流程端到端測試
    - `pairing-flow.test.ts`: 3 tests — pair.requested→pair.approved, duplicate rejection, non-existent pair rejection
    - 驗證配對狀態正確轉換（DB pair_id query for approved flow）
    - _需求：22.1_

  - [x] 18.3 實作 E2E 訊息交換端到端測試
    - `encrypted-messaging.test.ts`: 3 tests — encrypt→relay→decrypt round-trip, non-active pair rejection, not-in-pairing rejection
    - 使用 X25519 + HKDF-SHA-256 + XChaCha20-Poly1305 full E2E encryption
    - _需求：22.1, 22.2_

  - [x] 18.4 實作斷線重連端到端測試
    - `reconnect-catchup.test.ts`: 3 tests — missed events catchup, empty catchup, no catchup without seq
    - connectAndAuth 支援 lastSeenServerSeq 參數
    - _需求：22.3_

  - [x] 18.5 實作安全場景端到端測試
    - `security-scenarios.test.ts`: 5 tests — replay idempotency, tampered sig, pending pair relay, revoked pair relay, tampered payload
    - _需求：22.4_

- [x] 19. 最終 Checkpoint — 確認所有 MVP 測試通過
  - [x] 確保所有單元測試、屬性測試、端到端測試通過：408/408 tests, 63 files, all 4 gates green (typecheck/lint/test/format)
  - [x] 確認需求覆蓋：所有 Phase 0+1 需求（1-12, 21-23, 25-26）皆有對應任務與測試覆蓋（17/17 requirements verified）
  - [x] 無未解決疑問

---

## Phase 1.5：Web-First Usability（瀏覽器自助啟動）

> **動機**：MVP Phase 0+1 後端/協議/測試全部完成（408 tests），但使用者無法實際使用——Web UI 依賴 OpenClaw Plugin pipeline 建立 agent，普通使用者沒有 OpenClaw Gateway，導致冷啟動必然空白。此階段目標：令使用者「打開瀏覽器就能用」。
>
> **安全不變式**：Private key 永不離開瀏覽器（除非明確 DEMO_MODE）；Server/DB 不存 private key；E2E 格式沿用既有設計（X25519 + HKDF-SHA-256 + XChaCha20-Poly1305）；不破壞既有 Plugin pipeline。

- [x] 20. Browser Self-Bootstrap（瀏覽器自助註冊 — 最優先）✅ 440 tests（Session Claude_20260303_1030）
  - **DB 就緒狀態**：`owners` 表已存在（id, handle, pubkey, createdAt, updatedAt）；`agents.ownerId` FK 可為 null（支援 agent-first 模式）。無需 schema migration。
  - **身份架構決策**：採用 Agent-first PoP（Phase 0），Owner claim / email / OAuth 延後至 Phase 2+。瀏覽器 keypair = agent 身份。Private key 永不上傳 server。
  - [x] 20.1 實作 GET /api/auth/nonce 端點
    - 回傳 32-byte random nonce（hex）
    - Server 端以 Map 快取 nonce，TTL 5 分鐘，一次性使用（驗證後即刪除）
    - 限速：每 IP 每分鐘 ≤ 10 次
  - [x] 20.2 實作 POST /api/auth/bootstrap 端點（Proof-of-Possession）
    - 接收：pubkey (Ed25519 hex) + signature (對 `agentverse:<nonce>` 簽名) + 可選 AgentCard 欄位 (display_name, persona_tags)
    - 驗證：nonce 存在 + 未過期 + 簽名正確 → 刪除 nonce
    - 若 pubkey 首次出現：建立 agent + AgentCard（owners 表暫不建立 row，延後至 Owner claim phase）
    - 若 pubkey 已存在：查找既有 agent，更新 last login
    - 簽發 JWT（claims: `{ sub: agent_id, pubkey, scope: "agent", iat, exp }`，exp = 24h）
    - 回傳 `{ jwt, agent_id, agent_card, is_new }`
    - 限速：每 IP 每分鐘 ≤ 5 次（比 nonce 更嚴格）
    - 保留既有 admin secret flow 不變（向後相容）
  - [x] 20.3 Web UI 登入/註冊流程改造
    - 瀏覽器端生成 Ed25519 keypair（@noble/curves，pure JS，browser-safe）
    - Private key 存入 localStorage（JSON: `{ version, privateKey, publicKey, createdAt }`）
    - 首次訪問：[Create My Agent] 按鈕 → nonce → 簽名 → bootstrap → 取得 JWT + agent_id
    - 回訪用戶（keypair 已存在）：自動 nonce → 簽名 → bootstrap → silent re-auth
    - JWT 存入 localStorage，AuthProvider 讀取並注入 API 請求 header
    - 登出 = 清除 JWT（keypair 保留，下次可重新認證）
    - Login page 3-state: registration / re-auth / session active
    - NavBar shows truncated agent ID when authenticated
  - [x] 20.4 JWT auth 層重構
    - authPlugin 設定 `request.identity: RequestIdentity`（AdminIdentity | AgentIdentity）
    - 同時接受 admin JWT 與 agent JWT（scope 欄位區分）
    - Legacy JWT `{ pubkey: "web-user" }` → fallback admin（向後相容）
    - Export types: AdminIdentity, AgentIdentity, RequestIdentity
  - [x] 20.5 測試
    - NonceStore: 6 unit tests
    - Auth routes (nonce + bootstrap): 18 tests (4 existing + 14 new)
    - Auth plugin identity: 10 tests (4 existing + 6 new)
    - Web crypto: 6 unit tests
    - Total: 408 → 440 (+32 new tests)
  - _需求：3, 5, 11_

- [x] 21. Web Pairing UX Glue（瀏覽器配對互動）
  - [x] 21.1 AgentDex agent card/detail panel 增加 [Pair] 按鈕
    - 點擊 → POST /api/pairings（使用 agent-scoped targetAgentId 端點）
    - 顯示配對狀態回饋（PENDING / PAIRED / INCOMING REQUEST）
    - Self-pair, DEMO guard, ownership guards on approve/cancel/revoke
  - [x] 21.2 Pairings 頁面增強：incoming/outgoing/active 三區 + Accept/Reject/Cancel/Revoke 按鈕
    - 區分「我發起的」vs「對我發起的」配對請求
    - 解析 counterpart agent 名稱
  - [x] 21.3 後端測試：agent-scoped auth, DEMO guard, self-pair guard, ownership guards（16 new tests, 465 total）
  - **驗證日期：2026-03-03，465/465 tests，typecheck + lint + test + format:check 全綠**
  - _需求：7_

- [x] 22. Web Chat with E2E Encryption（瀏覽器端到端加密聊天）
  - [x] 22.1 重構 `packages/shared/src/e2e.ts` 支援瀏覽器
    - 方案：替換 libsodium-wrappers 為 @noble/ciphers（純 JS，瀏覽器安全）
    - Wire format 完全不變（XChaCha20-Poly1305 + nonce 前綴）
    - Crypto SSOT 交叉驗證：24 個 deterministic test vectors（7 個類別）
  - [x] 22.2 瀏覽器端 WebSocket 連線
    - ws-client.ts: 狀態機 WS client（challenge-response auth, ping/pong, exponential backoff）
    - WS↔Hub integration test: 4 tests 驗證 browser auth 協議相容性
  - [x] 22.3 Chat 頁面 UI
    - 左側：active pair list（從配對中選擇聊天對象）
    - 右側：message log（終端機風格，非氣泡）+ input box
    - 發送：ed25519KeyToX25519(peerPub) → encryptChat() → submit msg.relay
    - 接收：WS event → decryptChat() → append to log
    - [SECURE] E2E 指示器 + 連線狀態 badge
  - [x] 22.4 測試：ws-client (7), e2e-helpers (5), envelope-builder (3), cross-verify (24), ws-compat (4) = 43 new tests; 506 total
  - **驗證日期：2026-03-03，506 tests，typecheck + lint + format:check 全綠（P5 PBT flake 為既有問題）**
  - _需求：8, 22_

- [x] 23. Seed / Demo Mode（避免空白畫面）
  - [x] 23.1 新增 SEED_DEMO env flag + demo-agents seeder (4 agents, pubkey upsert, idempotent)
    - App boot 時若 flag=true：upsert 4 個展示用 AgentCard (Echo-7, Cipher-X, Luma, Navigator-9)
    - Seed agents 只有 metadata，無 private key（deterministic hex pubkeys）
    - UI 標示 `[ DEMO ]` badge (yellow BBS style)，detail pane 顯示 SHOWCASE notice
  - [x] 23.2 測試：env flag parsing (2) + seeder tests (7) = 9 new tests; 449 total
  - _需求：6_

- [x] 24. Phase 1.5 Checkpoint
  - [x] 確認 Task 20-23 全部通過
  - [x] 既有 408 tests 不退化（507 total, +99 new tests across Phase 1.5）
  - [x] Acceptance checklist:
    1. ✅ Fresh DB：Web UI 可自助建立 agent（無需 admin secret / plugin）
    2. ✅ AgentDex 不再空白（seed 4 demo agents + 自建 agent 可見）
    3. ✅ 兩端 Web session 可完成配對（REST API pair request + approve）
    4. ✅ 兩端 Web session 可 E2E 聊天（XChaCha20-Poly1305, server 不見明文）
    5. ✅ 既有 plugin pipeline + tests 全綠
  - [x] Contract consolidation: PROJECT_MASTER_SPEC §4.2 updated (@noble/ciphers), §4.3 Auth Contract, §4.4 Deployment Boundary
  - **驗證日期：2026-03-03，507/507 tests (71 files)，typecheck + lint + test + format:check 全綠**

---

## Phase 2.0：Prompt Brawl（Human×Agent PvP）

> **動機**：Phase 1.5 已有 AgentDex/Pairings/Chat，但缺乏「用途/流程/留存理由」。Phase 2 將三者整合成一個有明確目的、可重玩、可沉澱資產的體驗——**Prompt Brawl**：人類做教練，微操 prompt；Agent 代表出戰；勝負 deterministic 判定。
>
> **核心設計決策**：
>
> - 勝負判定 = regex/字串匹配（deterministic，不依賴 LLM 裁判）
> - Hub 不讀明文（E2E 不變），結算靠雙方簽名共識
> - Browser-first（延續 Phase 1.5）；Plugin mode 為 Phase 2+ optional
> - Client-side LLM calls（用戶自帶 API key）維持 E2E purity
>
> **Non-goals（明確排除）**：
>
> - LLM 裁判 / embeddings similarity / semantic scoring
> - Server-side LLM inference / key vault
> - 仲裁 / 上訴 / 第三方審判
> - GenePack 交易（Phase 3）
> - Elo rating（MVP 只做 wins/losses）

### Batch A — Match Orchestration（後端）

- [x] 25. 實作 Prompt Brawl 後端（Match Orchestration） ✅ 558 tests
  - [x] 25.1 定義 Trials 共用型別（`packages/shared`）
    - 新增 event types + payload interfaces（Ref: MASTER_SPEC §16.2 Event Types）
    - 新增型別 `TrialRule`（Ref: MASTER_SPEC §16.6 — 含 id, type, pattern, display_hint, difficulty）
    - 新增型別 `Verdict` + `SignedVerdict`（Ref: MASTER_SPEC §16.3 Verdict Payload & Signing）
    - DoD: 型別通過 typecheck，與既有 EventType/EventPayload 系統整合，欄位與 §16.3/§16.6 完全一致
  - [x] 25.2 DB migration（trials 相關表）
    - `trials` 表：id(uuid), pair_id(FK), rule_id, rule_payload(jsonb), seed(hex), status(created/started/reported/settled/timeout), created_by(agent_id FK), created_at, started_at, settled_at
    - `trial_results` 表：id(uuid), trial_id(FK unique), winner_agent_id(FK), loser_agent_id(FK), rule_id, trigger_event_id, transcript_digest(hex), sig_winner(hex), sig_loser(hex), settled_at
    - `agent_stats` 表：agent_id(FK PK), wins(int default 0), losses(int default 0), xp(int default 0), updated_at
    - 更新既有空殼 `trials_reports` 表或替換為新 schema
    - DoD: migration 跑通 + pg-mem 測試
  - [x] 25.3 TrialsRepository + AgentStatsRepository
    - TrialsRepository: createTrial, getTrial, updateStatus, getByPairId
    - TrialResultsRepository: createResult, getByTrialId
    - AgentStatsRepository: getStats, incrementWins, incrementLosses, addXp
    - DoD: 每個 repository ≥ 5 unit tests
  - [x] 25.4 Hub 結算邏輯（`trials.reported` → `trials.settled`）
    - 實作 Settlement Protocol（Ref: MASTER_SPEC §16.4 — 驗證步驟 1-5）
    - 驗簽通過 → 寫入 trial_results + 更新 agent_stats + 廣播 `trials.settled`
    - 驗簽失敗 → 回傳 `settlement_failed` + reason（Ref: §16.4 step 5 error codes）
    - DoD: ≥ 8 tests（happy path + sig mismatch + verdict mismatch + wrong trial state + wrong agents）
  - [x] 25.5 Rules Engine（規則集）
    - 預定義 rule set in `packages/shared/src/trial-rules.ts`（Ref: MASTER_SPEC §16.6 Trial Rules — type/pattern/difficulty 定義）
    - `selectRule(seed)`: deterministic 選取（seed-based，Ref: §16.6 seed → rule 映射）
    - `evaluateRule(rule, text)`: 回傳 `{ triggered: boolean, matchedAt?: number }`
    - DoD: ≥ 10 tests（各種規則 + edge cases + unicode）
  - [x] 25.6 Checkpoint — Batch A
    - 所有 Batch A tests 通過 + 全域 regression green
    - Integration test: 兩 agent 建立 match → both report (dual-sign) → settled → stats updated

### Batch B — Browser Trials Runner

- [x] 26. 實作 Browser Trials Runner ✅ 609 tests (Phase 0+1)
  - [x] 26.1 Match 狀態機（browser-side）
    - States: idle → challenge_sent → waiting_peer → ready → in_progress → judging → reporting → settled
    - Turn timer（可配 15s/30s default）
    - Turn timeout = forfeit（timeout 方自動 lose）
    - 雙方同步收到 rule（透過 `trials.created` event 的 rule_payload）
    - DoD: 狀態機 unit tests ≥ 8
  - [x] 26.2 Coach Console 介面
    - 輸入：「教練指令」（戰術 prompt text）
    - 組裝：`system_prompt + coach_instruction + conversation_context` → LLM prompt
    - LLM 呼叫：`LlmProvider` interface + `MinimaxProvider` MVP 實作（Ref: MASTER_SPEC §16.8 Client-side LLM Integration）
    - 生成結果 → E2E 加密 → msg.relay 發送
    - DoD: LlmProvider interface + mock provider tests ≥ 5
  - [x] 26.3 Rule Evaluation（browser-side）
    - 對每條收到的解密明文執行 `evaluateRule()`（Ref: MASTER_SPEC §16.6 rule types + evaluation 邏輯）
    - 觸發 → 本地生成 verdict + 簽名
    - DoD: evaluation tests ≥ 6（forbidden_word + regex + unicode + no-match + partial）
  - [x] 26.4 Transcript Digest Chain
    - 實作 digest 算法（Ref: MASTER_SPEC §16.5 — 初始值 + 遞推公式 + Hub-visible 欄位定義）
    - 兩端基於 Hub-visible 資料獨立計算 → 必須一致
    - DoD: ≥ 5 tests（empty chain, single msg, multi msg, tampered, deterministic cross-verify）
  - [x] 26.5 Verdict 簽名與上報
    - `buildVerdict()` + Ed25519 簽名（Ref: MASTER_SPEC §16.3 Verdict Payload & Signing — 欄位 + 簽名格式）
    - 協調協議（Ref: MASTER_SPEC §16.4 Settlement Protocol — 雙方簽名 → 組裝 `trials.reported`）
    - DoD: ≥ 6 tests（signing, verification, mismatch detection）
  - [x] 26.6 Checkpoint — Batch B
    - 全域 regression green
    - Integration: 兩 browser session 可完成一局（create → start → exchange messages → rule triggered → verdict signed → reported → settled）

### Batch C — Arena UI

- [x] 27. 實作 Prompt Brawl Arena 頁面 ✅ 615 tests (Phase 2)
  - [x] 27.1 Arena 頁面骨架（`app/arena/page.tsx`）
    - 上方狀態列：回合數、倒數計時、當前禁語提示（可選遮罩：只顯示 `_ _ _ _ 包`）
    - 中央：對話流（沿用 Chat 終端機 log 風格，cyan self / yellow peer / dimmed system）
    - 左側：Coach Console（prompt input + [SEND STRATEGY] button）
    - 系統訊息：MATCH STARTED / PENDING SETTLEMENT / VICTORY CONFIRMED / DEFEAT
    - BBS/GBA 鐵則：border-radius:0、硬陰影、ANSI Blue 背景、像素字體
    - DoD: 頁面渲染 + mock match 流程可手動操作
  - [x] 27.2 HP/危險度視覺指示器
    - Deterministic heuristic：n-gram overlap / edit distance 對禁語的接近度
    - 純視覺張力（progress bar style），**不影響勝負**
    - BBS style：ASCII art meter `[████████░░] 80%`
    - DoD: heuristic tests ≥ 4 + 視覺元件渲染
  - [x] 27.3 結算與結果展示
    - 勝負動畫（BBS style：`>>> VICTORY <<<` / `>>> DEFEAT <<<` 閃爍）
    - Stats 更新提示（+XP, +wins）
    - Badge 獲得通知（first_match / first_win / first_defense）
    - [REMATCH] / [BACK TO AGENTDEX] 按鈕
    - DoD: 結算 UI 可正確顯示各種結果
  - [x] 27.4 AgentDex 戰鬥入口整合
    - AgentCard 新增 stats 顯示（W/L record）
    - Detail panel 新增 [CHALLENGE] 按鈕（限 active pairing）
    - Challenge → 建立 trial → 跳轉 Arena
    - DoD: AgentDex → Arena 流程通暢
  - [x] 27.5 Checkpoint — Batch C
    - 全域 regression green
    - UAT: 完整 match 可在 Arena UI 中完成（配對 → 挑戰 → 對戰 → 結算 → 結果展示）

### Batch D — Progression（最小留存循環）

- [x] 28. 實作基礎成長系統
  - [x] 28.1 XP 計算與 agent_stats API
    - Win = +100 XP, Loss = +25 XP（參與獎勵）
    - GET /api/agents/:id/stats（公開）
    - Stats 嵌入 AgentCard API response（wins, losses, xp）
    - DoD: API + 4 tests
  - [x] 28.2 Badge 系統
    - 3 個 MVP badges: `badge_first_match`（完成首場）、`badge_first_win`（首勝）、`badge_first_defense`（首次防守成功=對手觸發禁語）
    - Badge 獎勵在 `trials.settled` 處理時自動檢查並授予
    - Badge 存入 agent metadata（`badges: string[]`）
    - DoD: badge 授予邏輯 + 5 tests
  - [x] 28.3 AgentDex Stats 展示
    - AgentCard 顯示 W-L record + XP level
    - Badge 圖示（複用既有 badge PNG assets）
    - DoD: 視覺整合
  - [x] 28.4 LLM Provider 設定 UI
    - Settings 頁面：API key 管理（localStorage, never sent to server）
    - Provider: MiniMax M2.5 (BYOK transitional)
    - DoD: 設定可存取，Arena 可使用
  - [x] 28.5 Phase 2.0 Checkpoint
    - 全域 regression green（typecheck + lint + test + format:check）
    - Acceptance checklist:
      1. ✅ 兩位用戶在瀏覽器端完成：配對 → 挑戰 → 對戰 → 本地判定觸發 → 雙方簽名上報 → Hub settle → UI 結果
      2. ✅ 勝負判定 deterministic（regex/string），不依賴 LLM 裁判
      3. ✅ Hub 不需解密明文亦可驗簽結算
      4. ✅ UI 完全符合 BBS/GBA 規格
      5. ✅ XP + wins/losses + 3 badges 正確授予
      6. ✅ Regression gates 全綠

---

## Phase 3+ Backlog [FROZEN — 2026-03-05 決策]

> **凍結原因**：GenePack 交換實用價值不足（skill 可從 ClawHub 取得，trait 由主人直接設定，knowledge 只是 markdown 檔案）。開發過分偏重規格撰寫，未能產出可玩 demo。方向調整為純 Fun Layer 社交遊戲平台。
>
> 以下任務保留作為歷史記錄，不會在當前 Sprint 計劃中排入。如需重啟，需 Founder (Adam) 明確批准。

- [FROZEN] B3. GenePack 交換流程（Phase 3）
  - **Why-defer**: GenePack 交換涉及三種類型（skill/trait/knowledge）的 Agent-to-Agent 交換流程，需 Phase 2 社交基礎設施（配對、E2E 訊息）穩定後實作
  - **Landing**: Phase 3（Phase 2 社交基礎設施穩定後）
  - GenePack schema 已定義（見 PROJECT_MASTER_SPEC §17，對齊 requirements.md 詞彙表）
  - 三種類型：skill（ClawHub/GitHub 指向）、trait（Brain Docs 配置，平台獨有）、knowledge（知識領域種子，平台獨有）
  - 交換流程：genepack.offered → genepack.accepted → 本地安裝建議 → 主人明確批准後安裝（requirements.md 需求 13）
  - 預設 private；public 需明確確認；僅交換指針與 metadata，不含檔案內容
  - **DoD**: 已配對 Agent 間可提出/接受 GenePack 交換，metadata 上傳成功且不含檔案內容，主人批准後安裝生效
  - _需求：13, 14_

- [FROZEN] B4. Lineage 家族樹事件記錄與 UI 視覺化（Phase 3）
  - **Why-defer**: 家族樹需要 GenePack 安裝/傳承事件作為數據源，GenePack 本身尚未實作（B3 前置依賴）
  - **Landing**: Phase 3（B3 完成後）
  - **DoD**: 視覺化元件可展示 agent 的 GenePack 傳承樹，含來源 agent 與安裝時間軸
  - _需求：16, 18_

- [FROZEN] B5. Fusion Lab 合成介面（Phase 3）
  - **Why-defer**: 合成需要多個 GenePack 存在，Phase 2 尚無 GenePack 系統（B3/B4 前置依賴）
  - **Landing**: Phase 3（B3+B4 完成後）
  - **DoD**: 用戶可選擇 ≥2 個 GenePacks 進行合成，產出新 GenePack 並記錄 lineage
  - _需求：24_

- [FROZEN] B6. 進階反濫用與信任層級（Post-MVP）
  - **Why-defer**: MVP 社群規模小（封閉配對），現有 LLM safety filters + E2E 已足夠；進階機制需真實濫用數據驅動設計
  - **Landing**: Post-MVP / Phase 3+（視社群規模與實際濫用情況決定）
  - **DoD**: 舉報機制 + client-side content warning + GenePack review gate 皆可運作
  - _需求：19_

- [FROZEN] B7. 資料匯出與刪除（Post-MVP）
  - **Why-defer**: Phase 2 資料量有限且無隱私法規強制要求；優先完成核心遊戲循環
  - **Landing**: Post-MVP / Phase 3+（上線前必做）
  - **DoD**: 用戶可匯出自己的 agent 數據（JSON）並刪除帳號，刪除後資料不可恢復
  - _需求：20_

### Phase 2 Non-goals（明確排除，附 defer 理由）

| Item                  | Why-defer                                                       | Landing Phase                      | DoD                                          |
| --------------------- | --------------------------------------------------------------- | ---------------------------------- | -------------------------------------------- |
| Elo rating            | MVP wins/losses 已足夠排名體驗；Elo 需大量對戰數據校準 K-factor | Phase 3+（累積 ≥500 場對戰數據後） | Elo 分數可計算、顯示、且 K-factor 經數據校準 |
| CSP headers           | React 自動 escape + E2E 架構已防 XSS；CSP 為防禦縱深非急迫      | Phase 2.5（上線前 hardening）      | CSP header 正確設定且不阻擋正常功能          |
| Server-side LLM       | 違反 E2E purity 原則；client-side API key 模式已滿足 MVP        | Not planned（架構決策）            | N/A                                          |
| Arbitration / Appeals | 雙簽共識已防單方作弊；仲裁需第三方可信節點，MVP 無此基礎設施    | Phase 3+（視社群需求）             | 可信第三方可審計 transcript 並裁定爭議       |
...
- 使用者提出的要點已分別嵌入：
  ...
  9. **openclaw-main pre-commit/CI gate**：見硬約束段落第 5 條；mount script 已棄用（第 6 條），改用 OpenClaw 原生 plugin loading
