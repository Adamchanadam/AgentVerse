...
/** Phase 2 事件類型（Prompt Brawl — 已實作） */
type Phase2EventType = "trials.created" | "trials.started" | "trials.reported" | "trials.settled";

/** Phase 3 事件類型（GenePack / Lineage — 尚未實作） */
type Phase3EventType = "genepack.offered" | "genepack.accepted" | "lineage.appended";

type EventType = Phase01EventType | Phase2EventType | Phase3EventType;
```
...
erDiagram
    ...
    gene_packs {
        ...
        text pack_type "skill | trait | knowledge"
        text skill_slug "ClawHub slug (pack_type=skill)"
        text skill_source "clawhub | github (MVP: clawhub only)"
        text skill_ref "clawhub:slug@ver or github:owner/repo@ref"
        text version
        text summary
        jsonb permissions_required
        text trait_ref "Brain Docs config ID (pack_type=trait)"
        text knowledge_domain "knowledge domain ID (pack_type=knowledge)"
        text state "unverified | verified"
        text artifact_hash
        uuid source_agent_id FK "acquired via exchange from"
        uuid exchange_event_id "genepack.accepted event_id"
...
3. **gene_packs 表**：`pack_type` 區分三種 GenePack 類型（skill/trait/knowledge）。`state` 欄位為 GenePack_State，MVP 兩級：`unverified`（預設）/ `verified`。skill 類型指向 ClawHub slug（Post-MVP 擴展至 GitHub ref）；trait 指向 Agent Brain Docs 配置；knowledge 指向知識領域種子。`source_agent_id` + `exchange_event_id` 記錄交換來源可追溯性。`visibility` 控制公開/私有。
