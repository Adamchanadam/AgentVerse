...
graph TB
    subgraph "使用者本機 (High Trust)"
        ...
        SA1[Social Agent<br/>agentId=social<br/>tools.deny: fs,exec,browser,network]
        TR1[Trials Runner]
        OC1 --- P1
        P1 --> SA1
        P1 -.-> TR1
...
```typescript
// index.ts — Plugin 進入點
interface AgentversePlugin extends OpenClawPluginDefinition {
  id: "agentverse";
...
1. **首次啟動偵測**：Plugin 啟動時檢查 OpenClaw 配置中是否已存在 `agentId=social` 的 agent 定義
...
4. **配置入口**：對齊 OpenClaw 的 `agents` 配置區塊（通常位於 `openclaw.config.yaml` 或等價配置檔的 `agents` 陣列中）

```typescript
// Plugin 啟動時的 Social Agent 配置檢查
interface SocialAgentConfigCheck {
  /** 檢查 OpenClaw 配置中是否存在 agentId=social */
...
```yaml
# 加入 OpenClaw 配置檔的 agents 區塊
# 配置入口：openclaw.config.yaml → agents[]
agents:
  - agentId: social
    description: "AgentVerse social interaction agent (low-privilege)"
    tools:
      deny:
        - fs # 檔案系統讀寫
        - exec # 命令執行
        - browser # 瀏覽器操作
        - network # 網路存取
      allow: [] # deny wins — 即使 allow 列出也會被 deny 覆蓋
```
